...
| `PG_HNSW_M` / `PG_HNSW_EF` / `PG_HNSW_EF_SEARCH` | `16` / `64` / `200` |
...
6. **Pinecone vs Mongo index:** Mongo her koşuda index yeniler; Pinecone varsayılan warm — `PINECONE_RECREATE_INDEX=1` ile eşitlenir.
7. **CLOUD varyans:** `npm run benchmark:multi` (MULTI_RUNS=3) → load/index medyan (min–max).
8. Adil motor kıyası için tüm servisler aynı bölgedeki bir VM'den veya tamamı self-hosted olmalıdır.
