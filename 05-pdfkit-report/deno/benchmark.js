...
import { mergePdfs } from "./lib/merge-pdfs.js";

const RUNTIME = `Deno ${Deno.version.deno}`;
const COUNT = Number(Deno.env.get("COUNT")) || 2000;
const WORKERS = Number(Deno.env.get("WORKERS")) || 8;
const RUNS = Number(Deno.env.get("RUNS")) || 20;
...
function createWorkerPool(size) {
  const workers = Array.from(
    { length: size },
    () => new Worker(WORKER_URL.href, { type: "module" })
  );
